...
7. **出 backport 卡 + 落盘**:`export_patch` 落盘适配后的补丁 + `export_report(topic=<bug 短标识>)` 落盘 backport 卡(topic 防同仓多主题报告互相覆盖)。**落完用 bash 把补丁和报告各 `cp` 一份到你会话的工作目录(用户打开 opencode 的目录),两个绝对路径都报给用户**——默认落点在 RootRecall 数据目录(`data/bug_rca/`),不在会话目录放副本 = 用户找不到 = 没交付;用户另指定目录就传 `out_dir`。**到这一步止,先别 memorize**。
...
| `rootrecall_export_patch(repo_path, out_dir)` | step 7 落盘 | 把 v20 工作区的适配改动写成补丁;落完 bash `cp` 一份到会话工作目录,两个绝对路径都报给用户 |
| `rootrecall_export_report(content, repo_path, topic=<bug 短标识>)` | step 7 落盘 | 写 backport 卡 .md;topic 防同仓覆盖;副本纪律同 export_patch |
